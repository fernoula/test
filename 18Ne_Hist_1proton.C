...
//int main()
{
  ...
  CS *cross = new CS;
  cross->ReadFile("cross_high.txt");
  cross->CrossSection<Simulator>(21.96,S);
  getchar();
  cross->CrossSection<Simulator>(0.7535,S);
  getchar();
  cross->CrossSection<Simulator>(50.0,S);
  getchar();
  cross->Thickness<Simulator>(S);
  getchar();
  ...
  TH1F* grBeamRec = new TH1F("grBeamRec","",175,0,15);
  ...
  */
  ...
  Double_t e=72.3;    //Beam energy range max after passing Kapton foil
  ...
  int j=0;
  const Int_t n=1000;
  const Int_t BeamParticles = 20000;
